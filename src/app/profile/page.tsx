...
import { PhotoCamera, LockReset, Article, Delete, FavoriteBorder, ChatBubbleOutline, MoreVert, People } from '@mui/icons-material'
...
import { changePassword, followUserAndUnfollow, getFollowSuggestions, getMyPosts, getMyProfile, uploadProfilePhoto, getUserProfile } from '../../lib/authSlice'
import toast from 'react-hot-toast'

export default function Profile() {
  const auth = useSelector((state: ReturnType<typeof store.getState>) => state.auth)
  const { myProfile, myPosts, followSuggestions, userProfile } = useSelector((state: ReturnType<typeof store.getState>) => state.auth)
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4, mb: 4 }}>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 4 }}>
        ...
        >
          <Tab icon={<Article />} label="My Posts" iconPosition="start" sx={{ fontWeight: 'bold' }} />
          <Tab icon={<People />} label="Following" iconPosition="start" sx={{ fontWeight: 'bold' }} />
          <Tab icon={<LockReset />} label="Security" iconPosition="start" sx={{ fontWeight: 'bold' }} />
        </Tabs>
      </Box>
          ...
          <Box sx={{ mb: 6 }}>
            ...
            <Box
              sx={{
                ...
                '&::-webkit-scrollbar-thumb': { backgroundColor: '#dcdcdc', borderRadius: 4 },
              ...
              {followSuggestions?.data?.suggestions.map((suggestion) => (
                ...
                >
                  <UserAvatar
                    ...
                    sx={{ width: 70, height: 70, mb: 1.5, border: '2px solid', borderColor: 'primary.light', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}
                  />
                  <Typography variant="subtitle2" fontWeight="700" noWrap sx={{ width: '100%' }}>
                    {suggestion.name}
                  </Typography>
                  <Typography variant="caption" color="text.secondary" display="block" noWrap sx={{ width: '100%', mb: 1 }}>
                    @{suggestion.username}
                  </Typography>
                  <Box sx={{ mt: 'auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 0.5, mb: 1.5 }}>
                  ...
                  <Button
                    ...
                    sx={{ width: '100%', borderRadius: 20, textTransform: 'none', fontWeight: 'bold' }}
                  >
                    Follow
                  </Button>
                </Card>
      ...
      {tabIndex === 1 && (
        <Box>
          ...
          {(!userProfile?.data?.user?.following || userProfile.data.user.following.length === 0) ? (
            <Card variant="outlined" sx={{ borderRadius: 4, p: 4, textAlign: 'center', borderColor: 'divider', bgcolor: '#fafafa' }}>
              <Typography variant="h2" color="primary.main" fontWeight="900" sx={{ mb: 1 }}>
                0
              </Typography>
      ...
      {tabIndex === 2 && (
        <Box sx={{ display: 'flex', justifyContent: 'center' }}>
          <Card variant="outlined" sx={{ borderRadius: 3, width: '100%', maxWidth: 500, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            <CardContent sx={{ p: 4 }}>
              ...
              <form onSubmit={formik.handleSubmit}>
                ...
                >
                  {auth.isLoading ? <CircularProgress size={24} color="inherit" /> : 'Update Password'}
                </Button>
              </form>
            </CardContent>
          </Card>
